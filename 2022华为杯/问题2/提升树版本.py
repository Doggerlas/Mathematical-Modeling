...
for mon in [0, 1, 2]:  # 1~3月
    ...
    # 提升树构造月份出现的次数与气候因素之间的关系模型
    zf_model = GradientBoostingClassifier()
    zf_model.fit(time, zf)  # 月份出现的次数与蒸发量之间关系模型
    qw_model = GradientBoostingClassifier()
    qw_model.fit(time, qw)  # 月份出现的次数与气温之间关系模型
    js_model = GradientBoostingClassifier()
    js_model.fit(time, js)  # 月份出现的次数与降水量之间关系模型
    qy_model = GradientBoostingClassifier()
    qy_model.fit(time, qy)  # 月份出现的次数与气压之间关系模型
    fs_model = GradientBoostingClassifier()
    fs_model.fit(time, fs)  # 月份出现的次数与风速之间关系模型

    # 提升树构造气候因素与不同深度土壤湿度之间的关系模型
    sd10_model = GradientBoostingClassifier()
    sd10_model.fit(Factor, sd10)  # 10cm湿度预测模型
    sd40_model = GradientBoostingClassifier()
    sd40_model.fit(Factor, sd40)  # 40cm湿度预测模型
    sd100_model = GradientBoostingClassifier()
    sd100_model.fit(Factor, sd100)  # 100cm湿度预测模型
    sd200_model = GradientBoostingClassifier()
